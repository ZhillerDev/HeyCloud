...
public class MinioUtils {
  ...
  private static String formatSize(long sizeInBits) {
    ...
    if (sizeInBits < KB) {
      // 小于1KB，直接以比特显示
      return String.format("%d bits", sizeInBits);
    } else if (sizeInBits < MB) {
      // 介于1KB和1MB之间，以KB为单位
      return String.format("%.2f KB", sizeInBits / KB);
    } else if (sizeInBits < GB) {
      // 介于1MB和1GB之间，以MB为单位
      return String.format("%.2f MB", sizeInBits / MB);
    } else {
      // 大于等于1GB，以GB为单位
      return String.format("%.2f GB", sizeInBits / GB);
    }
  }
   ...
   * @return 二进制流
   */
  public LinkedList<Result<Item>> listObjects(String bucketName, String prefix, boolean recursive) {
    LinkedList<Result<Item>> res = new LinkedList<>();
    Iterable<Result<Item>> results = minioClient.listObjects(
